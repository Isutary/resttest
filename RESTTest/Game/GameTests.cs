...
namespace RESTTest.Game
{
    ...
    {
        public GameTests() : base(CommonConstants.RestClient.GameService) { }
        ...
        {
            Init(Constants.Game, Method.POST);
            string open = DateTime.Now.AddMinutes(1).ToString("yyyy-MM-ddTHH:mm:ssZ");
            string end = DateTime.Now.AddMinutes(1).ToString("yyyy-MM-ddTHH:mm:ssZ");
        ...
        {
            Init(Constants.Game, Method.POST);
            string open = DateTime.Now.AddMinutes(5).ToString("yyyy-MM-ddTHH:mm:ssZ");
            string end = DateTime.Now.AddMinutes(8).ToString("yyyy-MM-ddTHH:mm:ssZ");
            request.AddJsonBody(new AddGameRequest(code, first, consolation, open, end, recurring, pattern));
        ...
        public void GameTests_Start_And_End_Must_Be_Same(string code, string first, string consolation, string recurring, string pattern)
        {
            Init(Constants.Game, Method.POST);
            string open = DateTime.Now.AddMinutes(5).ToString("yyyy-MM-ddTHH:mm:ssZ");
            string end = DateTime.Now.AddMinutes(15).ToString("yyyy-MM-ddTHH:mm:ssZ");
            request.AddJsonBody(new AddGameRequest(code, first, consolation, open, end, recurring, pattern));
        ...
        public void GameTests_Incorrect_Recurring(string code, string first, string consolation, string recurring, string pattern)
        {
            Init(Constants.Game, Method.POST);
            string open = DateTime.Now.AddMinutes(5).ToString("yyyy-MM-ddTHH:mm:ssZ");
            string end = DateTime.Now.AddMinutes(5).ToString("yyyy-MM-ddTHH:mm:ssZ");
            request.AddJsonBody(new AddGameRequest(code, first, consolation, open, end, recurring, pattern));
